...
def reached_goal_bt():
	...
	while True:
		e = yield {waitFor: forward_action}
		obs, info = e.data["observation"], e.data["info"]
		if goal_pos == get_agent_position(obs, info):
...
def doorkey_env_distance_bt(bt_obs: BThreadObservation):
	...
	while True:
		...
		bt_obs.update_observation(distance)
		e = yield {waitFor: EventList([forward_action, picked_up_key, dropped_key, opened_door, closed_door, passed_door_left, passed_door_right])}
...
internal_bthreads_doorkey = [pick_up_key_bt,
					 drop_key_bt,
					 reached_goal_bt,
					 unlock_door_bt,
					 open_door_bt,
...
observable_bthreads_doorkey = [
								doorkey_env_level_bt,
								doorkey_env_distance_bt,
								]
...
bthreads = {
	...
	"MiniGrid-DoorKey-6x6-v0": (internal_bthreads_doorkey, observable_bthreads_doorkey),
	"MiniGrid-DoorKey-8x8-v0": (internal_bthreads_doorkey, observable_bthreads_doorkey),
}
